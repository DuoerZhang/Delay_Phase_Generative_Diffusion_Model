...
def find_classes_unsort(dir):
    classes = [d for d in os.listdir(dir) if os.path.isdir(os.path.join(dir, d))]
    class_to_idx = {classes[i]: classes[i] for i in range(len(classes))}
    return classes, class_to_idx
...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        # print('self._total_size',self._total_size)
        # print('self.img_resolution',self.img_resolution)
        # self._raw_shape = [self._total_size] + [3, resolution, resolution]
        self.raw_shape = [self._total_size] + [1, self.img_resolution, self.img_resolution]
        self.xflip = super_kwargs["xflip"]
        self.return_vid = return_vid
    ...
    def __getitem__(self, index):
        ...
        # 4 frames:input0-3,range[1,3] means using input1\2 for input3 predicting
        # further should be range[0,3] means using input0\1\2 for input3
        frames= [0, self.nframes-1]

        path0, target0,file_name0 = clip[frames[0]]
        img0 = self._load_nii_from_path(path0)

        path1, target1,file_name1 = clip[frames[0]+1]
        img1 = self._load_nii_from_path(path1)

        path2, target2,file_name2 = clip[max(frames) - 1]
        img2 = self._load_nii_from_path(path2)
        
        path3, target3,file_name3 = clip[max(frames)]
        img3 = self._load_nii_from_path(path3)
        path_mask = '/data/zder/ddpm_vidm/data/CT4_160cubeHUNorm01_hcc_icc_nii_test_mask0613/'+file_name0+'/mask2.nii'
        mask = self._load_nii_from_path(path_mask)
        mask[mask>0] = 1.0
        if self.apply_resize:
            img0 = resize_crop_img(img0, self.img_resolution)
            img1 = resize_crop_img(img1, self.img_resolution)
            img2 = resize_crop_img(img2, self.img_resolution)
            img3 = resize_crop_img(img3, self.img_resolution)
        if self.xflip and index >= self._total_size // 2:
            img0 = torch.flip(img0, dims=[2])
            img1 = torch.flip(img1, dims=[2])
            img2 = torch.flip(img2, dims=[2])
            img3 = torch.flip(img3, dims=[2])
        # range 0-1 to [-1,1]
        img0 = img0 * 2. - 1.
        img1 = img1 * 2. - 1.
        img2 = img2 * 2. - 1.
        img3 = img3 * 2. - 1.
        assert target0 == target1 == target2 == target3
        assert file_name0 == file_name1 == file_name2 == file_name3

        return img0, img1, img2, img3, file_name0, mask#max(frames) - min(frames)

    def __len__(self):
        return self._total_size


# when sample, don't change index
class ImageSampleDataset(Dataset):
    def __init__(self,
        ...
        classes, class_to_idx = find_classes_unsort(path)
        ...
        self._path = path
        self._total_size = len(self.imgs) * 2 if super_kwargs["xflip"] else len(self.imgs)
        self.raw_shape = [self._total_size] + [3, self.img_resolution, self.img_resolution]
        self.xflip = super_kwargs["xflip"]
        self.return_vid = return_vid
        self.shuffle_indices = [i for i in range(self._total_size)]
        self.to_tensor = transforms.ToTensor()
        ...
        # PIL.Image.init()
        # for xx in self._all_fnames:
        #     print(xx)
        #     30/input1.nii

        #     print(self._file_ext(xx))
        #     .nii
    ...
    def __getitem__(self, index):
        ...
        frames= [0, self.nframes-1]
        # frames.sort()
        path0, target0 = clip[0]
        img0 = self._load_nii_from_path(path0)

        path1, target1 = clip[1]
        img1 = self._load_nii_from_path(path1)
        
        path2, target2 = clip[2]
        img2 = self._load_nii_from_path(path2)

        path3, target3 = clip[3]
        img3 = self._load_nii_from_path(path3)
        ...
        return img0, img1, img2, img3, target0, max(frames) - min(frames)
